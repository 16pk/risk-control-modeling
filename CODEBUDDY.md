...
信贷风控建模专家包（ModelEvo 框架）：一个 Agent 定义 + 精简后 11 个建模 Skill 的专家包，端到端交付可上线的风控分类模型（XGBoost / LightGBM）。
...
Python 3.10+；建模依赖 xgboost / lightgbm / scikit-learn / pandas / numpy / pyarrow / matplotlib / joblib。
...
- `model-skills/{name}/` — 每个 skill：`SKILL.md`（frontmatter `name` == 目录名）+ `scripts/` + `references/` + `tests/`。
  ...
  - `classification-model-experiments` — **v2.3 起主链路默认训练模块**（原独立实验台）：样本×特征正交矩阵 + 对抗验证 + **winner 规则诊断（置于 Optuna 前并驱动搜索锚点）** + Optuna 调优 + 转正；仅消费 `sample.parquet` + `feature-list.csv` + `model.split`。**红线例外（用户授权）：对抗格/IV-PSI 格 OOT 可参与对抗训练与筛选统计（禁早停/禁进训练/禁结构选择），OOT 指标标注乐观偏差**。v2.7 起 `classification-model-training` / `classification-model-tuning` / `classification-model-comparison` 已完全移除（DNN/LR 评分卡、单模型调参、N-way 深度对比能力随之永久下线）。
...
- **切分唯一真相** = `feature_config.yaml` 的 `model.split`；experiments 主链路开发池=train+test 合并后 seed=42 随机 70/30 切 train/val（无独立 test 档），不落 session 级 `splits/`。
...
- 当前（plugin.json `version`，v2.7.0）：单编排器（development）+ 主链路（需求澄清 → 特征列识别 feature-classification → 清洗 → 特征分析 → experiments 实验矩阵+对抗+规则诊断+Optuna+转正 → 默认打分）；training/tuning/comparison 三模块已完全移除（算法收敛 lgb/xgb）；已支持定版模型打分（model-scoring 默认执行，仅 lgb/xgb）与 FICO 转换（score-to-fico，可选）。
