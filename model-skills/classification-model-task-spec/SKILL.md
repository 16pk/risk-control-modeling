...
> **v2.1 精简重构**：本 skill 只做需求澄清，不产独立样本分析报告、不做 Gate P0 引擎裁决（本地文件唯一链路无需分布式裁决）、不做三档切分。输出收敛为 `task-spec.md`（单文件）+ `_manifest.json`（断点续跑）。
...
| 3 | **Train/Test/OOT 切分窗口？** | 三档起止日期（YYYY-MM-DD 或 8 位 YYYYMMDD），或样本起止 + 比例；**OOT 必须按时间顺序且晚于训练窗** | 仅记录/透传到 `feature_config.yaml` 的 `model.split`，切分由消费方（training/tuning/evaluation）即时进行 |
...
3. 切分窗口：Train / Test / OOT 三档起止日期（OOT 必须晚于训练窗），或样本起止日期 + 比例
...
2. 三档时序递增且互不相交（Train < Test < OOT）
3. **OOT 必须按时间顺序且晚于训练窗**（跨时间稳定性检验是上线前置）
...
`validate_split_ranges` 校验失败（三档缺失 / 起>止 / 时序不递增）→ 报错指明问题，让用户修正后重跑。
