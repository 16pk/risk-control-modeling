...
description: 建模 pipeline 内部的特征分析环节（development Stage 0）：对样本宽表里的候选特征做基础统计 / 单变量预测力 IV+AUC / 训练-OOT 稳定性 PSI，按配置切分 Train/Test/OOT 并产 splits，**切分前自动把哨兵值(-1/-2/-999/-9999 等)替换为 NaN，切分后自动剔除 label 缺失/非法样本**，仅产报告不自动剔特征。仅由 classification-model-development 编排调起，不响应独立关键词触发；用户主动发起样本/特征分析请用 credit-data-analysis。
---
...
| `--invalid-values` | 否 | `None`(取 yaml `model.invalid_values`,再默认哨兵集合) | 哨兵值集合(逗号分隔,如 `-1,-2,-999,-9999`),覆盖 yaml;传空串 `''` 关闭 |
...
├── report.md               # 主交付; 人工阅读(含「五、无效值哨兵替换」段)
...
└── invalid-values-report.csv  # 哨兵值替换明细 (feature/hit_values/n_hit/hit_ratio; 空表=未命中)
...
| 哨兵值替换 | `invalid-values-report.csv` | **切分前自动执行**:入模特征命中哨兵值(默认 -1/-2/-9/-99/-999/-9999/-99999,可配 `model.invalid_values` / CLI `--invalid-values`)时替换为 NaN;记录 feature/hit_values/n_hit/hit_ratio;report.md 渲染「五、无效值哨兵替换」段提醒 |
| 标签缺失剔除 | 切分时自动执行 | **切分后剔除 label 缺失/非法(非 0/1)行**:标签缺失样本无法参与训练与评估(尤其 OOT 评估,AUC/KS 会因 NaN 报错或口径污染),三档统一剔除;评估侧另有防御剔除兜底 |
| 主报告 | `report.md` | 一、概述 二、基础统计表 三、IV+单变量 AUC 排序 三-bis WOE 分桶明细(IV Top 20) 四、PSI(含 WARN 标记) 五、无效值哨兵替换 |
...
| ⚠️ 哨兵值替换是**值级清洗**非特征筛选 | 哨兵值替换(命中值→NaN)是切分前自动执行的数据准备动作,与"剔特征"是两回事:前者不减少特征数,只把占位值视为缺失;特征是否剔除仍由人工/`classification-model-tuning` 决定。若业务上某特征哨兵值有真实语义(如趋势类 -1/0/1),请在 `model.invalid_values` 中排除该值或传 `[]` 关闭 |
