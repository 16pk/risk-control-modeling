...
def test_creates_dir_and_files(tmp_path, fake_script):
    ...
    entry = record_stage(str(session), "training", fake_script, "python run_build.py --version v1")
    assert entry is not None
    assert (session / "scripts" / "training" / "run_build.py").exists()
    assert (session / "scripts" / "training" / "command.json").exists()
    assert (session / "scripts" / "_manifest.json").exists()
    # 快照内容与源一致
    assert (session / "scripts" / "training" / "run_build.py").read_text(encoding="utf-8") \
        == "# fake entry script\nprint('hi')\n"
...
def test_command_json_fields(tmp_path, fake_script):
    ...
    session = tmp_path / "session"
    record_stage(str(session), "training", fake_script, "python run_build.py --version v1",
                 label="Stage1 模型训练")
    cmd_json = _read_json(session / "scripts" / "training" / "command.json")
    assert cmd_json["stage"] == "training"
    assert cmd_json["cmd"] == "python run_build.py --version v1"
    assert cmd_json["label"] == "Stage1 模型训练"
    assert cmd_json["script_path"] == os.path.abspath(fake_script)
    assert cmd_json["snapshot"] == "training/run_build.py"
...
def test_manifest_merges_multiple_stages(tmp_path, fake_script):
    ...
    session = tmp_path / "session"
    record_stage(str(session), "training", fake_script, "python run_build.py")
    other = tmp_path / "run_tuning.py"
    other.write_text("print('tune')\n", encoding="utf-8")
    record_stage(str(session), "tuning", str(other), "python run_tuning.py")
    manifest = _read_json(session / "scripts" / "_manifest.json")
    assert set(manifest["stages"].keys()) == {"training", "tuning"}
    assert manifest["stages"]["training"][0]["snapshot"] == "training/run_build.py"
    assert manifest["stages"]["tuning"][0]["snapshot"] == "tuning/run_tuning.py"


def test_same_script_idempotent_override(tmp_path, fake_script):
    """同名脚本重复记录: 快照被覆盖为最新内容, manifest 条目更新而非无限追加。"""
    session = tmp_path / "session"
    record_stage(str(session), "training", fake_script, "cmd v1")
    # 修改源脚本内容后再次记录
    with open(fake_script, "w", encoding="utf-8") as f:
        f.write("# v2\nprint('new')\n")
    record_stage(str(session), "training", fake_script, "cmd v2")
    snap = session / "scripts" / "training" / "run_build.py"
    assert snap.read_text(encoding="utf-8") == "# v2\nprint('new')\n"
    manifest = _read_json(session / "scripts" / "_manifest.json")
    assert len(manifest["stages"]["training"]) == 1  # 覆盖旧条目, 不追加
    assert manifest["stages"]["training"][0]["cmd"] == "cmd v2"


def test_same_stage_different_scripts_appends(tmp_path, fake_script):
    """同一 stage 不同脚本 → manifest 中该 stage 追加为新条目。"""
    session = tmp_path / "session"
    record_stage(str(session), "tuning", fake_script, "cmd a")
    another = tmp_path / "select_features.py"
    another.write_text("print('select')\n", encoding="utf-8")
    record_stage(str(session), "tuning", str(another), "cmd b")
    manifest = _read_json(session / "scripts" / "_manifest.json")
    assert len(manifest["stages"]["tuning"]) == 2
...
def test_script_missing_raises(tmp_path):
    ...
    with pytest.raises(ValueError, match="不存在"):
        record_stage(str(tmp_path / "s"), "training", str(tmp_path / "nope.py"), "cmd")
...
def test_non_py_script_raises(tmp_path):
    ...
    with pytest.raises(ValueError, match=".py"):
        record_stage(str(tmp_path / "s"), "training", str(p), "cmd")
...
def test_unwritable_session_warns_not_blocking(tmp_path, fake_script):
    ...
    try:
        entry = record_stage(str(session), "training", fake_script, "cmd")
