...
**Session 决议**(由上游负责):本 skill 假定 `session_dir` 已由上游 `classification-model-development` 确定。若用户直接调本 skill 且无 session 上下文,请先跑:
...
2. **predictions/explainability 为中间产物**：评估依赖 predictions 作输入（内部必需）；用户需要打分明细时显式要求再交付。
3. **切分单一真相**：三档切分 = `model.split` 区间（feature_config.yaml / train_config.yaml），由训练消费时即时切分（run 内部 `data/splits/`），不落 session 级 `splits/`。
4. 单 run 产物结构见下方各节，实际文件不减少（保证脚本链路稳定）。
...
- 单模型标准化评估:见 `evaluation/{run_name}_{split}_eval.md`(由本 skill 内嵌 `eval_single.py` 产出)。
